...
Para poder administrar dispositivos móviles de Windows Phone con Intune, tendrá que configurar los requisitos de administración. Con la [creación de un CNAME de DNS](set-up-windows-phone-management-with-microsoft-intune.md) es más fácil que los usuarios se conecten al portal de empresa de Intune. Windows Phone 8.0 pide un certificado de Symantec para establecer una conexión de IP cifrada entre los dispositivos e Intune. También es necesario un certificado para firmar aplicaciones de línea de negocio.
...
-   **Windows Phone 8**: necesario

![Diagrama de los requisitos del certificado](../media/wpcertreqs.png)

  > [!IMPORTANT]
  > [Debe renovar periódicamente](renew-a-symantec-code-signing-certificate.md) el certificado de Symantec que se usa para administrar determinados dispositivos móviles de Windows y Windows Phone..
...
1.  **Configurar Intune**
    Si aún no lo ha hecho, prepárese para la administración de dispositivos móviles. Para ello, [defina la entidad de administración de dispositivos móviles](https://technet.microsoft.com/library/mt346013.aspx) como **Microsoft Intune** y configure MDM.
...
2.  **Establecer un alias DNS para la dirección del servidor de inscripción** (opcional)
    ...
    2.  Escriba la dirección URL del dominio verificado del sitio web de empresa en el cuadro **Especificar un nombre de dominio verificado** y, después, haga clic en **Probar detección automática**..
    ...
    3.  Debe crear registros de recursos DNS **CNAME** para el dominio de su empresa. Los registros de recursos CNAME deben contener la siguiente información:
        ...
        -   `manage.microsoft.com` – Admite un redireccionamiento al servicio Intune con reconocimiento de dominio del nombre de dominio del correo electrónico.
...
3.  **Administración de certificados para admitir la firma de aplicaciones**
    [Requerido para los dispositivos de Windows Phone 8.0 y Windows Phone 8.1 que no tendrán acceso a la Tienda de Windows Phone o que necesitan aplicaciones de línea de negocio].
    ...
    5.  **Exportar un certificado de firma**
        ...
        En el **Asistente para exportar certificados**, seleccione **Exportar la clave privada** y haga clic en **Siguiente**..  Seleccione **Intercambio de información personal: PKCS #12 (.PFX)** y active **Include all the certificates in the certification path if possible** (Incluir todos los certificados en la ruta de certificación si es posible). Complete el asistente. Para más información, vea [Exportar un certificado con la clave privada](http://go.microsoft.com/fwlink/?LinkID=203031)..
    ...
    6.  **Descargar y firmar la aplicación Portal de empresa**
        ...
        1.  **Descargar Portal de empresa**
            Descargue la aplicación [Portal de empresa de Intune para Windows Phone](http://go.microsoft.com/fwlink/?LinkId=268440) desde el centro de descarga. La ubicación de instalación predeterminada es `C:\Program Files (x86)\Microsoft Corporation\Windows Intune Company Portal for Windows Phone`.

        2.  **Descargar el SDK de Windows Phone 8.0**
            Descargar el [SDK de Windows Phone](http://go.microsoft.com/fwlink/?LinkId=615570).

        3.  **Firmar el código de la aplicación de portal de empresa**
            Use la aplicación XAPSignTool descargada con el SDK para firmar el portal de empresa con el archivo .pfx creado mediante el certificado de Symantec. Para más información, vea [How to sign a company app by using XapSignTool](http://go.microsoft.com/fwlink/?LinkID=280195) (Cómo firmar una aplicación de empresa mediante XapSignTool)..

    7.  **Cargar la aplicación de portal de empresa en Intune**
        Cargue el archivo de la aplicación de portal de empresa firmado y su certificado de firma de código para que la aplicación esté disponible para los usuarios finales.
...
4.  **Indique a los usuarios cómo obtener acceso a los recursos de empresa con el Portal de empresa**
...
2.  **Descargar el SDK de Windows Phone**
    Descargue el [SDK de Windows Phone 8.0](http://go.microsoft.com/fwlink/?LinkId=615570) (http://go.microsoft.com/fwlink/?LinkId=268439) e instálelo en el equipo. Este SDK es necesario para generar un token de inscripción de aplicaciones.

3.  **Generar un archivo AETX**
    Genere un token de inscripción de aplicaciones (.aetx) a partir del archivo PFX de Symantec con AETGenerator.exe, parte del SDK de Windows Phone 8.0. Para obtener instrucciones sobre cómo crear un archivo AETX, consulte [Cómo generar un token de inscripción de aplicaciones para Windows Phone](https://msdn.microsoft.com/library/windows/apps/jj735576.aspx).

4.  **Descargar SDK de Windows para Windows 8.1**
...
5.  **Firmar el código de la aplicación mediante PowerShell**
    Como administrador, abra **Windows PowerShell** en el equipo host donde están instalados el SDK de Windows y el certificado de firma de código móvil empresarial de Symantec, desplácese hasta el archivo Sign-WinPhoneCompanyPortal.ps1 y ejecute el script.
