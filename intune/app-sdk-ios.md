...
## <a name="whats-in-the-sdk-repository"></a>Qué hay en el repositorio del SDK

Los siguientes archivos son relevantes para las aplicaciones/extensiones que no contienen código SWIFT, o bien se compilan con una versión de Xcode anterior a la 10,2:

* **IntuneMAM.framework**: marco del SDK de aplicaciones Intune. Se recomienda vincular este marco de trabajo a su aplicación o extensiones para habilitar la administración de aplicaciones cliente de Intune. Sin embargo, algunos desarrolladores pueden preferir las ventajas de rendimiento de la biblioteca estática. Vea las opciones siguientes.

* **libIntuneMAM.a**: biblioteca estática del SDK de aplicaciones de Intune. Los desarrolladores pueden optar por vincular la biblioteca estática en lugar del marco de trabajo. Dado que las bibliotecas estáticas se incrustan directamente en el archivo binario de la aplicación o la extensión en tiempo de compilación, hay algunas ventajas de rendimiento en tiempo de ejecución para usar la biblioteca estática. Sin embargo, integrarlo en su aplicación es un proceso más complicado. Si la aplicación incluye extensiones, la vinculación de la biblioteca estática a la aplicación y las extensiones dará como resultado un mayor tamaño de agrupación de aplicaciones, ya que la biblioteca estática se incrustará en cada archivo binario de aplicación o extensión. Al usar el marco de trabajo, las aplicaciones y extensiones pueden compartir el mismo archivo binario del SDK de Intune, lo que da lugar a un tamaño de aplicación más pequeño.

* **IntuneMAMResources.bundle**: lote de recursos que contiene los recursos en los que se basa el SDK. El paquete de recursos solo es necesario para las aplicaciones que integran la biblioteca estática (libIntuneMAM. a).

Los siguientes archivos son relevantes para las aplicaciones/extensiones que contienen código SWIFT y se compilan con Xcode 10.2 +:

* **IntuneMAMSwift. Framework**: el marco de trabajo SWIFT del SDK para aplicaciones de Intune. Este marco de trabajo contiene todos los encabezados de las API a las que llamará la aplicación. Vincule este marco de trabajo a su aplicación o extensiones para habilitar la administración de aplicaciones cliente de Intune.

* **IntuneMAMSwiftStub. Framework**: marco de código auxiliar SWIFT del SDK para aplicaciones de Intune. Esta es una dependencia necesaria de IntuneMAMSwift. Framework que las aplicaciones y extensiones deben vincular.


Los siguientes archivos son relevantes para todas las aplicaciones y extensiones:
...
* **Encabezados**: expone las API públicas de Intune App SDK. Estos encabezados se incluyen en los marcos de IntuneMAM/IntuneMAMSwift, por lo que los desarrolladores que consumen cualquiera de los marcos de trabajo no necesitan agregar manualmente los encabezados a su proyecto. Los desarrolladores que decidan vincular con la biblioteca estática (libIntuneMAM. a) deberán incluir manualmente estos encabezados en el proyecto.
...
1. **Opción 1: marco de trabajo (recomendado)** : Si usa Xcode 10.2 y su aplicación o extensión contiene `IntuneMAMSwift.framework` código SWIFT, vínculo y `IntuneMAMSwiftStub.framework` a su destino: arrastre `IntuneMAMSwift.framework` y `IntuneMAMSwiftStub.framework` a la lista de **archivos binarios insertados** del destino del proyecto.

    De lo contrario `IntuneMAM.framework` , vincule a su destino `IntuneMAM.framework` : arrastre a la lista de **archivos binarios incrustados** del destino del proyecto.
   ...
   **Opción 2: biblioteca estática**: esta opción solo está disponible para las aplicaciones/extensiones que no contienen código SWIFT o que se crearon con Xcode < 10,2. vincular a la biblioteca `libIntuneMAM.a`. Arrastre la bibloteca `libIntuneMAM.a` en la lista de **Linked Frameworks and Libraries** (Marcos y bibliotecas vinculados) del destino del proyecto.
...
Si su aplicación aún no usa ADAL o MSAL y no necesita acceder a ningún recurso de AAD, no es necesario configurar un registro de aplicación cliente en AAD si decide integrar ADAL. Si decide integrar MSAL, deberá configurar un registro de aplicaciones e invalidar el identificador de cliente de Intune y el URI de redireccionamiento predeterminados.  
...
### <a name="link-to-adal-or-msal-binaries"></a>Vínculo a los archivos binarios de ADAL o MSAL

**Opción 1:** Siga [estos pasos](https://github.com/AzureAD/azure-activedirectory-library-for-objc#download) para vincular la aplicación a los archivos binarios de Adal.

**Opción 2:** Como alternativa, puede seguir [estas instrucciones](https://github.com/AzureAD/microsoft-authentication-library-for-objc#installation) para vincular la aplicación a los archivos binarios de MSAL.
...
Como se mencionó anteriormente, el SDK para aplicaciones de Intune puede usar la [biblioteca de autenticación de Azure Active Directory](https://github.com/AzureAD/azure-activedirectory-library-for-objc) o la biblioteca de autenticación de [Microsoft](https://github.com/AzureAD/microsoft-authentication-library-for-objc) para sus escenarios de inicio condicional y de autenticación. También se basa en ADAL/MSAL para registrar la identidad del usuario con el servicio de MAM para la administración sin escenarios de inscripción de dispositivos. Si **la aplicación no usa Adal o MSAL para su propio mecanismo de autenticación**, es posible que deba configurar las opciones de AAD personalizadas, en función de la biblioteca de autenticación que elija integrar:   

ADAL: Intune App SDK proporcionará valores predeterminados para parámetros ADAL y controlará la autenticación en Azure AD. No es necesario que los desarrolladores especifiquen ningún valor para la configuración de ADAL mencionada anteriormente. 

MSAL: los desarrolladores deben crear un registro de aplicación en AAD con un URI de redireccionamiento personalizado en el formato especificado [aquí](https://github.com/AzureAD/microsoft-authentication-library-for-objc/wiki/Migrating-from-ADAL-Objective-C-to-MSAL-Objective-C#app-registration-migration). Los desarrolladores deben establecer `ADALClientID` los `ADALRedirectUri` valores de y que se mencionaron anteriormente `aadRedirectUriOverride` , o las `IntuneMAMPolicyManager` propiedades y equivalentes `aadClientIdOverride` de la instancia. Los desarrolladores también deben asegurarse de que siguen el paso 4 de la sección anterior, para conceder al registro de la aplicación acceso al servicio de protección de aplicaciones de Intune.

### <a name="special-considerations-when-using-msal"></a>Consideraciones especiales al usar MSAL 

1. **Compruebe su WebView** : se recomienda que las aplicaciones no usen SFSafariViewController, SFAuthSession o ASWebAuthSession como su WebView para las operaciones de autenticación interactiva de MSAL iniciadas por la aplicación. Si, por alguna razón, la aplicación debe usar una de estas vistas de WebView para cualquier operación interactiva de autenticación de MSAL `SafariViewControllerBlockedOverride` , `true` también debe `IntuneMAMSettings` establecerse en en el Diccionario de info. plist de la aplicación. ADVERTENCIA: se desactivarán los enlaces de SafariViewController de Intune para habilitar la sesión de autenticación. Esto hace que se produzcan pérdidas de datos de riesgo en cualquier parte de la aplicación si la aplicación usa SafariViewController para ver los datos corporativos, por lo que la aplicación no debe mostrar los datos corporativos en ninguno de esos tipos WebView.
2. La **vinculación de Adal y MSAL** : los desarrolladores deben optar si desean que Intune prefiera la MSAL en Adal en este escenario. De forma predeterminada, Intune prefiere las versiones de ADAL admitidas a las versiones de MSAL compatibles, si ambas están vinculadas en tiempo de ejecución. Intune solo preferirá una versión de MSAL compatible cuando, en el momento de la primera operación de autenticación de `IntuneMAMUseMSALOnNextLaunch` Intune `NSUserDefaults`, se encuentra `true` en. Si `IntuneMAMUseMSALOnNextLaunch` es `false` o no se establece, Intune revertirá al comportamiento predeterminado. Como sugiere el nombre, `IntuneMAMUseMSALOnNextLaunch` se aplicará un cambio en el siguiente inicio.
...
Valor  | Tipo  | Definición | ¿Necesario?
--       |  --   |   --       |  --
ADALClientId  | Cadena  | Identificador de cliente de Azure AD de la aplicación. | Se requiere para todas las aplicaciones que usan MSAL y cualquier aplicación de ADAL que tenga acceso a un recurso de AAD que no sea de Intune. |
ADALAuthority | Cadena | Entidad de Azure AD de la aplicación en uso. Debe usar su propio entorno donde se hayan configurado cuentas de AAD. | Obligatorio si la aplicación usa ADAL o MSAL para tener acceso a un recurso de AAD que no es de Intune. Si este valor está ausente, se usa un valor predeterminado de Intune.|
ADALRedirectUri  | Cadena  | URI de redireccionamiento de Azure AD de la aplicación. | Se requiere ADALRedirectUri o ADALRedirectScheme para todas las aplicaciones que usan MSAL y cualquier aplicación ADAL que tenga acceso a un recurso de AAD que no sea de Intune.  |
ADALRedirectScheme  | Cadena  | Esquema de redireccionamiento de Azure AD de la aplicación. Puede usarse en lugar de ADALRedirectUri si el URI de redireccionamiento de la aplicación se encuentra en el formato `scheme://bundle_id`. | Se requiere ADALRedirectUri o ADALRedirectScheme para todas las aplicaciones que usan MSAL y cualquier aplicación ADAL que tenga acceso a un recurso de AAD que no sea de Intune. |
...
SafariViewControllerBlockedOverride | Booleano| Deshabilita los enlaces de SafariViewController de Intune para habilitar la autenticación de MSAL a través de SFSafariViewController, SFAuthSession o ASWebAuthSession. | Opcional. El valor predeterminado es No. ADVERTENCIA: puede producir la pérdida de datos si se usa de forma incorrecta. Habilite solo si es absolutamente necesario. Consulte [consideraciones especiales al usar MSAL](#special-considerations-when-using-msal) para más información.  |
...
ForegroundColor| Cadena| Especifica el color de primer plano para los paneles de PIN e inicio, como el color del texto. Acepta una cadena RGB hexadecimal con el formato #XXXXXX, donde las X pueden ir de 0 a 9 o de A a F. Se puede omitir la almohadilla.  | Opcional. El valor predeterminado es negro. |
AccentColor | Cadena| Especifica el color de énfasis de la pantalla de PIN, como el color de texto del botón y el color de resaltado del cuadro. Acepta una cadena RGB hexadecimal con el formato #XXXXXX, donde las X pueden ir de 0 a 9 o de A a F. Se puede omitir la almohadilla.| Opcional. El valor predeterminado es el azul del sistema. |
...
> [!Important]
> El SDK de aplicaciones de Intune para iOS usa claves de cifrado de 256 bits cuando el cifrado se habilita mediante las directivas de protección de aplicaciones. Todas las aplicaciones deberán tener una versión del SDK actual para permitir el uso compartido de datos protegidos.
...
Para obtener más información sobre cómo crear una directiva de configuración de aplicaciones de destino de MAM en iOS, vea la sección de la configuración de aplicaciones de destino de MAM en [How to use Microsoft Intune app configuration policies for iOS](app-configuration-policies-use-ios.md) (Cómo usar las directivas de configuración de aplicaciones de Microsoft Intune para iOS).
...
El SDK de Intune para iOS 9.0.3 + admite la posibilidad de agregar una consola de diagnóstico dentro de la aplicación móvil para probar las directivas y los errores de registro. `IntuneMAMDiagnosticConsole.h`define la `IntuneMAMDiagnosticConsole` interfaz de clase, que los desarrolladores pueden usar para mostrar la consola de diagnóstico de Intune. Esto permite a los usuarios finales o desarrolladores durante la prueba recopilar y compartir registros de Intune para ayudar a diagnosticar cualquier problema que puedan tener. Esta API es opcional para los integradores.
