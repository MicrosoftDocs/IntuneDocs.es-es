...
- **Bit locker removable drive policy** (Directiva de unidad extraíble de BitLocker)  
  ...
  Directiva de unidad extraíble de bits la caja de seguridad, configure las siguientes opciones:

    - **Require encryption for write access** (Requerir cifrado para el acceso de escritura)  
      **Default**: Sí  
  
    - **Método de cifrado**  
      **Default**: AES CBC de 256 bits  
...
- **Bit locker fixed drive policy** (Directiva de unidad fija de BitLocker)  
  Esta configuración de directiva se usa para controlar el método y la intensidad del cifrado. Los valores de esta directiva determinan la intensidad del cifrado que BitLocker usa para el cifrado. A las empresas les interesa controlar el nivel de cifrado para mejorar la seguridad (AES-256 es más seguro que AES-128). Si habilita esta directiva, puede configurar un algoritmo de cifrado y la intensidad de cifrado de clave para unidades de datos fijas, unidades del sistema operativo y unidades de datos extraíbles de manera individual. Para unidades del sistema operativo y unidades fijas, se recomienda usar el algoritmo XTS-AES. Para unidades extraíbles, debe usar el cifrado AES-CBC de 128 bits o AES-CBC de 256 bits si la unidad se usa en otros dispositivos que no ejecuten Windows 10 (versión 1511 o posterior). El cambio del método de cifrado no tendrá ningún efecto si la unidad ya está cifrada o si el cifrado está en curso. En estos casos, se omite esta configuración de directiva.  
   ...
   Para que BitLocker se ha corregido la directiva de unidad, configure las siguientes opciones: 
   - **Método de cifrado**
     **predeterminado**: AES de 256 bits XTS  

- **Bit locker system drive policy** (Directiva de unidad del sistema de BitLocker)  
  Esta configuración de directiva se usa para controlar el método y la intensidad del cifrado. Los valores de esta directiva determinan la intensidad del cifrado que BitLocker usa para el cifrado. A las empresas les interesa controlar el nivel de cifrado para mejorar la seguridad (AES-256 es más seguro que AES-128). Si habilita esta directiva, puede configurar un algoritmo de cifrado y la intensidad de cifrado de clave para unidades de datos fijas, unidades del sistema operativo y unidades de datos extraíbles de manera individual. Para unidades del sistema operativo y unidades fijas, se recomienda usar el algoritmo XTS-AES. Para unidades extraíbles, debe usar el cifrado AES-CBC de 128 bits o AES-CBC de 256 bits si la unidad se usa en otros dispositivos que no ejecuten Windows 10 (versión 1511 o posterior). El cambio del método de cifrado no tendrá ningún efecto si la unidad ya está cifrada o si el cifrado está en curso. En estos casos, se omite esta configuración de directiva.  

   Para la directiva de unidad de sistema de la caja de seguridad Bit, configure las siguientes opciones:
  - **Método de cifrado**  
    **Default**: AES de 256 bits XTS  
...
- **Credential Guard**  
  ...
  **Default**: habilitar con bloqueo UEFI 
...
- **Hardware device installation by device identifiers** (Instalación de dispositivos de hardware mediante identificadores de dispositivo)  
  ...
  **Valor predeterminado**: Bloquear la instalación de dispositivos de hardware  
    ...
    Cuando *bloquear la instalación de dispositivos de hardware* está activada, las siguientes opciones están disponibles.
...
- **Requerir contraseña**  
  ...
  **Default**: Sí  
    ...
    - **Password minimum character set count** (Número mínimo de conjuntos de caracteres de contraseña)  
      El número de elementos complejos (letras mayúsculas y minúsculas, números y signos de puntuación) necesarios para una contraseña o PIN fuerte. PIN aplica el comportamiento siguiente para escritorios y dispositivos móviles: 1 - dígitos sólo 2 - dígitos y letras minúsculas son necesarios 3 - dígitos, letras minúsculas, y letras en mayúsculas son necesarias. No se admite en cuentas de Microsoft de escritorio y cuentas de dominio. 4 - Se requieren dígitos, letras minúsculas, letras mayúsculas y caracteres especiales. No se admite en el escritorio. El valor predeterminado es 1. 
...
- **Bloquear el Contenido destacado de Windows**  
  ...
  **Default**: Sí  
  ...
  Cuando *contenido destacado de Windows del bloque* está establecido en *Sí*, las siguientes opciones están disponibles.
...
- **XML de protección contra vulnerabilidades**  
  ...
  **Default**: *se proporciona el xml de ejemplo* 
...
- **Internet Explorer internet zone allow only approved domains to use Active X controls** (Permitir que solo los dominios aprobados usen controles ActiveX de TDC en la zona de Internet de Internet Explorer)  
  Esta configuración de directiva controla si el usuario puede ejecutar el control ActiveX de CDC en sitios Web. Si habilita esta configuración de directiva, el control ActiveX de TDC no se ejecutará desde los sitios web de esta zona. Si deshabilita esta configuración de directiva, el control ActiveX de TDC se ejecutará desde todos los sitios de esta zona.
...
- **Internet Explorer omita las advertencias de SmartScreen sobre archivos poco comunes**  
...
- **Internet Explorer restringe los permisos de zona de java**  
...
- **Internet Explorer restricted zone logon options** (Opciones de inicio de sesión de la zona de sitios restringidos de Internet Explorer)  
  ...
  - *Inicio de sesión automático solo en la zona Intranet* -Utilice esta opción para consultar a los usuarios para los identificadores de usuario y contraseñas en otras zonas. Después de la consulta, estos valores se pueden usar de modo silencioso en lo que resta de la sesión. 
  - *Inicio de sesión automático con el nombre de usuario actual y la contraseña*-Utilice esta opción para intentar inician sesión con la respuesta de desafío de Windows NT (también conocida como autenticación NTLM). Si el servidor es compatible con la respuesta de desafío de Windows NT, el inicio de sesión usa la red nombre de usuario y contraseña para iniciar sesión. Si el servidor no es compatible con la respuesta de desafío de Windows NT, se consultará al usuario para que proporcione el nombre de usuario y la contraseña. 
...
- **No se ejecutan de zona de Internet Explorer intranet antimalware con controles Active X** esta configuración de directiva determina si Internet Explorer se ejecuta programas antimalware contra controles ActiveX, para comprobar si son seguras cargar en las páginas. Si habilita esta configuración de directiva, Internet Explorer no comprobará el programa antimalware para saber si se puede crear una instancia del control ActiveX de forma segura. Si deshabilita esta configuración de directiva, Internet Explorer siempre comprobará el programa antimalware para saber si se puede crear una instancia del control ActiveX con seguridad. Si no se establece esta configuración de directiva, Internet Explorer no comprobará el programa antimalware para saber si se puede crear una instancia del control ActiveX de forma segura. Los usuarios pueden activar o desactivar este comportamiento por medio de las opciones de seguridad de Internet Explorer.
...
- **Restrict anonymous access to named pipes and shares** (Restringir el acceso anónimo a las canalizaciones con nombre y los recursos compartidos)  
  Cuando se habilita, esta configuración de seguridad restringe el acceso anónimo a los recursos compartidos y a la configuración para: (1) canalizaciones con nombre pueden ser accesibles anónimamente (2) los recursos compartidos que se pueden acceder de forma anónima
...
- **Comportamiento de extracción de tarjeta inteligente**  
    Esta configuración de seguridad determina lo que sucede cuando la tarjeta inteligente de un usuario que haya iniciado sesión se extrae del lector de tarjetas inteligentes. Las opciones son:
     - *Ninguna acción*. 
     - *Bloquear la estación de trabajo* -la estación de trabajo está bloqueado cuando se quita la tarjeta inteligente, lo que permite a los usuarios abandonan el área, llevarse las tarjetas inteligentes con ellos y seguir manteniendo una sesión protegida.
  ...
  **Default**: Bloquear estación de trabajo
...
- **Require admin approval mode for administrators** (Requerir el modo de aprobación de administrador para administradores)  
  ...
  - *No configurado*: se deshabilitan el Modo de aprobación de administrador y todas las configuraciones de directiva de UAC relacionadas. Nota: Si se deshabilita esta configuración de directiva, el Centro de seguridad notifica que ha disminuido la seguridad global del sistema operativo. 
...
- **Prevent anonymous enumeration of SAM accounts** (Evitar la enumeración anónima de cuentas SAM)  
  ...
  - *No configurado*: sin restricciones adicionales. Basar en los permisos predeterminados.  
...
- **Usar Modo de aprobación de administrador**  
  ...
  - *No configurado*: la cuenta predefinida de administrador ejecuta todas las aplicaciones con privilegios de administración completos.  
...
- **SMB v1 client driver start configuration** (Configuración de inicio del controlador de cliente SMB v1)  
  **Default**: deshabilita el controlador
...
- **Standby states when sleeping while on battery** (Estados de espera mientras el equipo está en suspensión con batería)  
  Esta configuración de directiva administra si Windows pueden usar estados de espera al poner el equipo en un estado de suspensión. Si habilita o no establece esta configuración de directiva, Windows usará los estados de espera para poner el equipo en estado de suspensión. Si deshabilita esta configuración de directiva, no se permitirán los estados de espera (S1-S3).
...
- **System boot start driver initialization** (Inicialización de controladores de arranque del sistema)  
  ...
  - *Defectuoso*: el controlador se ha identificado como malware. Le recomendamos que no permita conocidos los malos conductores a inicializarse. 
...
- **Email content execution type** (Tipo de ejecución de contenido del mensaje de correo electrónico)  
  Esta regla bloquea los siguientes tipos de archivo que se ejecute o se inicia desde un correo electrónico en Microsoft Outlook u webmail (por ejemplo, Gmail.com o Outlook.com): archivos ejecutables (por ejemplo, .exe, .dll o .scr) archivos de Script (por ejemplo, un PowerShell .ps, VisualBasic .vbs, o un archivo de JavaScript .js) archivos de Script.
  
  **Default**: bloque
  
- **Tipo de protección de red**  
  Esta directiva permite activar o desactivar la protección de red (Bloquear o Auditar) en Protección contra vulnerabilidades de seguridad de Windows Defender. Protección de red es una característica de Protección contra vulnerabilidades de seguridad de Windows Defender que evita que los empleados que usan una aplicación accedan a estafas de suplantación de identidad, sitios que hospedan vulnerabilidades de seguridad y contenido malintencionado en Internet. Esto incluye impedir que exploradores de terceros se conecten a sitios peligrosos. El tipo de valor es entero. Si habilita esta configuración, se activa la protección de red y los empleados no podrán desactivarla. Este comportamiento se puede controlar mediante las opciones siguientes: Bloquear y Auditar. Si habilita esta directiva con la opción "Bloquear", se impide que usuarios y aplicaciones se conecten a dominios peligrosos. Puede ver esta actividad en el centro de seguridad de Windows Defender. Si habilita esta directiva con la opción "Auditar", no se impedirá que los usuarios o las aplicaciones se conecten a dominios peligrosos. Pero podrá seguir viendo esta actividad en el Centro de seguridad de Windows Defender. Si deshabilita esta directiva, no se impedirá que los usuarios o las aplicaciones se conecten a dominios peligrosos. No verá ninguna actividad de red en el Centro de seguridad de Windows Defender. Si no configura esta directiva, el bloqueo de red está desactivado de forma predeterminada.
...
- **Ink Workspace** (Área de trabajo de Ink)  
  ...
  - *No configurado* : el área de trabajo de Ink característica está activada y el usuario puede utilizar por encima de la pantalla de bloqueo.  
