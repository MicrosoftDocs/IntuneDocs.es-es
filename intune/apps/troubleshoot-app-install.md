...
En esta sección se menciona la inscripción por parte del administrador de dispositivos (DA) y mediante Samsung Knox. Para obtener más información, vea [Inscripción del administrador de dispositivos Android](../enrollment/android-enroll-device-administrator.md) e [Inscripción automática de dispositivos Android mediante Knox Mobile Enrollment de Samsung](../enrollment/android-samsung-knox-mobile-enroll.md). 
...
| No se pudo desinstalar la aplicación. (0xc7d14fb8) | Se trata de un error genérico de desinstalación. El sistema operativo no ha especificado el motivo por el que no se ha podido desinstalar la aplicación. Algunas aplicaciones de administración simplemente no se pueden desinstalar. Compruebe que la aplicación se puede desinstalar manualmente y recopile los registros de Portal de empresa si se produce un error al desinstalar. |
| El archivo de instalación (APK) de la aplicación usado para la actualización no coincide con la firma de la aplicación actual en el dispositivo. (0xc7d14fb7) | El sistema operativo Android tiene la limitación de exigir que el certificado de firma de la versión de actualización sea exactamente el mismo que el certificado usado para firmar la versión existente. Si el desarrollador no puede usar el mismo certificado para firmar la nueva versión, tiene que desinstalar la aplicación existente y volver a implementar la nueva aplicación en lugar de actualizar la existente. |
| El usuario final canceló la instalación de la aplicación. (0xc7d14fb9) | Indique al usuario que acepte la aplicación implementada de Intune y la instale cuando se le pida. |
| La desinstalación de la aplicación se ha cancelado porque el proceso se ha reiniciado durante la instalación. (0xc7d14fbc) | El sistema operativo ha finalizado el proceso de instalación de la aplicación o el dispositivo se ha reiniciado. Vuelva a intentar la instalación y recopile los registros de Portal de empresa si el error se produce de nuevo. |
| No se puede instalar el archivo de instalación (APK) de la aplicación porque no estaba firmado. (0xc7d14fb6) | De forma predeterminada, el sistema operativo Android exige que las aplicaciones se firmen. Asegúrese de que la aplicación está firmada antes de implementarla. |
...
| El usuario ha rechazado la oferta para actualizar la aplicación. (0x87D13B63) | El usuario final hizo clic en Cancelar durante el proceso de actualización. Implemente según sea necesario o indique al usuario que acepte la solicitud de actualización. |
...
| Error de Apple en la asignación de licencias: No quedan licencias de VPP (0x87d13b7e) | Este comportamiento es así por diseño. Para solucionarlo, adquiera licencias de VPP adicionales o reclame licencias a los usuarios a los que ya no van destinadas. |
| Error de instalación de la aplicación 12024: causa desconocida. (0x87d13b6e) | Apple no ha proporcionado información suficiente para determinar por qué se ha producido un error en la instalación. No hay nada que notificar. |
| La directiva de configuración de aplicaciones necesaria no está presente; asegúrese de que la directiva está destinada a los mismos grupos. (0x87d13b7f) | La aplicación requiere una configuración de aplicación, pero no hay ninguna objetivo. El administrador debe asegurarse de que los grupos de destino de la aplicación también tengan la configuración de aplicación necesaria destinada a los grupos. |
| La concesión de licencias VPP solo es aplicable para los dispositivos iOS 9.0+. (0x87d13b69) | Actualice los dispositivos iOS afectados a iOS 9.0+. |
| La aplicación está instalada en el dispositivo, pero no administrada. (0x87d13b8f) | Este error solo se produce en aplicaciones de LOB. La aplicación se ha instalado fuera de Intune. Para solucionar este error, desinstale la aplicación del dispositivo. La próxima vez que se produzca la sincronización del dispositivo, este debe instalar la aplicación desde Intune. |
...
| Error desconocido. (0x87d1279d) | Este error se produce en aplicaciones de la tienda iOS, pero se desconoce el escenario de error. |
| La versión más reciente de la aplicación no se pudo actualizar desde una versión anterior. (0x87D13B9D) | Este mensaje de error se muestra si la aplicación está instalada y administrada pero con la versión incorrecta en el dispositivo. Esta situación incluye si un dispositivo ha recibido un comando para actualizar una aplicación pero la nueva versión aún no se ha instalado y notificado. Este error se notifica en la primera sincronización de un dispositivo después de la implementación de la actualización y se produce hasta que el dispositivo notifica que la nueva versión está instalada, o se produce un error debido a otro error.  |
...
|  0x8000FFFF  |  Error inesperado durante la instalación. Compruebe los registros de instalación para obtener información adicional.  |
