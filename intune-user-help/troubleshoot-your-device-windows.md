---
title: Solucionar problemas habituales en el dispositivo Windows | Microsoft Docs
description: Descubra formas de solucionar problemas habituales en el dispositivo Windows.
keywords: 
author: barlanmsft
ms.author: barlan
manager: angrobe
ms.date: 05/24/2017
ms.topic: article
ms.prod: 
ms.service: microsoft-intune
ms.technology: 
ms.assetid: 48644443-ff50-4af1-9c23-443e6e5c3b47
searchScope: User help
ROBOTS: 
ms.reviewer: priyar
ms.suite: ems
ms.openlocfilehash: 526cfed5e8f42d739c57882b454745ba5b20181c
ms.sourcegitcommit: 34cfebfc1d8b81032f4d41869d74dda559e677e2
ms.translationtype: HT
ms.contentlocale: es-ES
ms.lasthandoff: 07/01/2017
---
# <a name="fix-common-issues-with-your-windows-device"></a>Solucionar problemas habituales en el dispositivo Windows

A veces pueden producirse problemas en el dispositivo Windows que se pueden corregir mediante la solución de problemas. A continuación se enumeran algunos problemas comunes que puede intentar resolver primero por su cuenta. Para problemas más complejos, se recomienda ponerse en contacto con el administrador de TI mediante su información de contacto en el [sitio web del Portal de empresa](https://portal.manage.microsoft.com).

- [El dispositivo Windows 10 no se inscribe](troubleshoot-your-windows-10-device-windows.md)
- [Tengo que actualizar el dispositivo Windows](you-need-to-update-your-windows-device.md)
- [Tengo que enviar registros al administrador de TI desde la aplicación Portal de empresa de Intune](send-logs-to-your-it-admin-cp-windows.md)
- [Tengo que enviar registros al administrador de TI desde la aplicación Configuración](send-logs-to-your-it-admin-settings-windows.md)
- [La versión de Windows todavía no es compatible](your-windows-version-isnt-yet-supported.md)
- [Tengo que habilitar el cifrado de Windows](you-need-to-enable-windows-encryption.md)
- [El dispositivo necesita software antimalware](your-device-needs-antimalware-software.md)
- [Tengo que habilitar el arranque seguro](you-need-to-enable-secure-boot-windows.md)
- [Tengo que habilitar la integridad de código](you-need-to-enable-code-integrity.md)
